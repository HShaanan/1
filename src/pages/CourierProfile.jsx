...
import { 
  ...
  MapPin, CheckCircle, User, AlertTriangle, ArrowRight,
  Phone, Mail, Briefcase, Car, Zap, Pencil, Save, X
} from 'lucide-react';
...
export default function CourierProfile() {
  ...
  // Get courier ID from URL params (for admin view)
  const urlParams = new URLSearchParams(window.location.search);
  const courierIdFromUrl = urlParams.get('id');

  useEffect(() => {
    loadData();
  }, [period, courierIdFromUrl]);
  ...
  const loadData = async () => {
    try {
      ...
      // If admin and has courier ID in URL, load that courier
      if (currentUser.role === 'admin' && courierIdFromUrl) {
        const couriers = await base44.entities.Courier.filter({ id: courierIdFromUrl });
        if (couriers && couriers.length > 0) {
          courierData = couriers[0];
        }
      } else {
        // For regular users - find courier by email
      ...
      let allDeliveries = [];
      try {
        allDeliveries = await base44.entities.DeliveryRecord.filter({ 
          courier_id: courierData.id 
        }, '-delivery_date', 200);
      } catch (e) {
        console.warn('No delivery records found');
        allDeliveries = [];
      }
  ...
  const handleUpdatePayment = async (deliveryId) => {
    try {
      await base44.entities.DeliveryRecord.update(deliveryId, {
  ...
  const getVehicleIcon = (type) => {
    switch(type) {
      ...
      case 'electric_bike': return <Zap size={16} />;
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-2 mt-3">
                <Badge variant="outline" className="gap-1">
                  ...
                  {getVehicleLabel(courier?.vehicle_type)}
                </Badge>
                <Badge variant="outline" className="gap-1">
                  <Briefcase size={12} />
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="text-left flex flex-col items-end gap-1">
                        ...
                        {/* Payment Amount - Editable for Admin */}
                        {editingDeliveryId === delivery.id && isAdmin ? (
                          <div className="flex items-center gap-1 mt-1">
                            ...
                            <Button 
                              ...
                              onClick={() => handleUpdatePayment(delivery.id)}
                            >
                              <Save size={12} />
                            </Button>
                            <Button 
                          ...
                          <div className="flex items-center gap-1">
                            <span className={`text-sm font-bold ${delivery.payment_amount > 0 ? 'text-green-600' : 'text-slate-400'}`}>
                            ...
                            {isAdmin && (
                              <Button 
                                ...
                                onClick={() => {
                                  ...
                                  setEditPaymentAmount(delivery.payment_amount || 0);
                                }}
                              >
                                <Pencil size={12} />
                              </Button>
                    ...
                    <div className="flex items-center gap-4 text-xs text-slate-500">
                      ...
                      {delivery.customer_address && (
                        <span className="flex items-center gap-1">
                          <MapPin size={12} />
                          {delivery.customer_address.length > 30 
